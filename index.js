...
// Añadir moneda y método getRandom
const moneda = 'coins';
...
function getMensajeSistema({ comando, verifyaleatorio, user2, edades }) {
  return {
...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    try {
      ...
      if (user) {
        // Inicializar todas las propiedades del usuario
        const userProps = {
          ...
          lastcodereg: 0, muto: false, premium: false, premiumTime: 0,
          registered: false, genre: '', birth: '', marry: '', description: '',
          packstickers: null, name: m.name, age: -1, regTime: -1, afk: -1,
          afkReason: '', banned: false, useDocument: false, bank: 0,
          level: 0, role: 'Nuv', warn: 0, crime: 0, antispam: 0,
          antispam2: 0, spam: 0
        };
        
        for (const [key, defaultValue] of Object.entries(userProps)) {
          if (!(key in user)) {
            if (typeof defaultValue === 'number') {
              if (!isNumber(user[key])) user[key] = defaultValue;
            } else {
        ...
        global.db.data.users[m.sender] = {
          ...
          crime: 0, antispam: 0, antispam2: 0, spam: 0
        };
      }
      ...
      if (chat) {
        const chatProps = {
          isBanned: false, sAutoresponder: '', welcome: false,
          autolevelup: false, autoresponder: false, delete: false,
          autoAceptar: false, autoRechazar: false, detect: true,
          economy: true, gacha: true, antiBot: false, antiBot2: false,
          modoadmin: false, antiLink: true, antifake: false,
          reaction: false, nsfw: false, expired: 0
        };
        
        for (const [key, defaultValue] of Object.entries(chatProps)) {
      ...
      if (settings) {
        ...
        for (const [key, defaultValue] of Object.entries(settingsProps)) {
          if (!(key in settings)) settings[key] = defaultValue;
        }
      } else {
    ...
    m.isWABusiness = this.authState?.creds?.platform === 'smba' || this.authState?.creds?.platform === 'smbi'
...
// LÍNEA CORREGIDA - El error principal
